...
//        this.installEditPolicy( new draw2d.policy.canvas.FadeoutDecorationPolicy());
        ...
        this.html.bind("mouseup touchend", $.proxy(function(event)
        {
            ...
            //GPLViewModel.mouseUpDetected = true;
            event = this._getEvent(event);
            this.calculateConnectionIntersection();
        ...
        {
            if (GPLViewModel.isViewer())return;
            //if (GPLViewModel.rightPanelOpen())return;
        ...
        this.html.bind("mousedown touchstart", $.proxy(function(event)
        {
            ...
            case 3: //Right mouse button pressed
                ...
                event = this._getEvent(event);
                pos = this.fromDocumentToCanvasCoordinate(event.clientX, event.clientY);
                var figure = this.getBestFigure(pos.x, pos.y);
                //console.log(figure);
                if(figure !==null) {
                    ...
                    if (!GPLViewModel.isViewer()) {
                        ...
                        if (typeof(figure.openPointReview) === 'function'){
                           figure.openPointReview();
                        }
                    }
...
//            this.mouseDownX = event.clientX;
//            this.mouseDownY = event.clientY;
//            var pos = this.fromDocumentToCanvasCoordinate(event.clientX + window.scrollX, event.clientY + window.scrollY);
...
//        $( "#filter" ).on( "panelclose", function( event, ui ) {
//            GPLViewModel.rightPanelOpen(false);
...
//            paintArea.height(this.initialHeight * this.zoomFactor);
   ...
   //     alert("dd");
    ...
    getBestFigure : function(x, y, figureToIgnore)
    {
        ...
        for ( i = 0; i < this.figures.getSize(); i++)
        {
            ...
            if (figure.isVisible()===true && figure.hitTest(x, y + $(window).scrollTop()) === true && figure !== figureToIgnore)
            {
                if (result === null){
                    result = figure;
                }
    ...
    * @template
    ...
    onKeyDown:function(keyCode, ctrl)
    {
      ...
      //PNG CTRL M
      else if (keyCode === 77 && ctrl){
          if(GPLViewModel.isViewer())
              return;
          event.preventDefault();
          //this.displayJSON(false);
          var writer = new draw2d.io.png.Writer();
          var svg = writer.marshal(this, false);
          $("#preview").html('').append(svg);
          //$('#preview').popup('open');

      }
      //CTRL A
      else if (keyCode === 65 && ctrl){
          if(GPLViewModel.isViewer())
              return;
          event.preventDefault();
          this.activateSequence(true);
      }
      //CTRL B
      else if (keyCode === 66 && ctrl){
          if(GPLViewModel.isViewer())
              return;
          event.preventDefault();
          this.saveAndPublish();
          //this.activateSequence();
      }
//      console.log(keyCode);
    },
