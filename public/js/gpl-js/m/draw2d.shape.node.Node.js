...
draw2d.shape.node.Node = draw2d.Figure.extend({
    ...
    */
    ...
    createPortWithTooltip: function(type, tooltip, enumOrFloat, locator, nam, alternateProperty, isRequired){
        var newPort = null;
        var count =0;
        ...
        switch(type){
            case "input":
                ...
                newPort.setDraggable(false);
                count = this.inputPorts.getSize();
                if (tooltip !== "Control Point")
        ...
        newPort.propertyRepresented = tooltip;
        //console.log(newPort);
        if (isRequired)
            newPort.setRequired(isRequired);

        if(alternateProperty)
            newPort.alternateProperty = alternateProperty;

        newPort.setValue(enumOrFloat);

        //newPort.setName(type+count);
        if(enumOrFloat == "Math")
        {
            console.log(enumOrFloat);
            console.log(locator);
        }
        newPort.setName(nam);
