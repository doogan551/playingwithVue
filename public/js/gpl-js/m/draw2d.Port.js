...
draw2d.Port = draw2d.shape.basic.Circle.extend({
    ...
    init : function( name)
    {
       ...
       // status var for user interaction
        ...
        this.ox = this.x;
        this.oy = this.y;
        this.coronaWidth = 1; // the corona width for the hitTest method. Usefull during drag&drop of ports. Better SnapTo behaviour.
    ...
    onClick:function(){
        //console.log('sss');
        //console.log(this.parent.onClick());
        this.parent.onClick();
    },
    ...
    onDrop:function(dropTarget)
    {
        ...
        if (this.getParent() instanceof draw2d.shape.node.IconObject && dropTarget.getParent() instanceof draw2d.shape.node.IconObject)
            return;
        ...
        //console.log(request.source.parent.shapeCat);

        if (request.source.parent.shapeCat == "Control")
        {
            s = this;
            t = dropTarget;
            if (s.getConnections().getSize() > 0)
            {
                iToast.showWarn('Warning', "Control Point port is already assigned.");
                return;
            }
            v = GPLViewModel.getProp(t.parent.id).upi();
        }
        else
        {
            v = GPLViewModel.getDProp(t.parent.id, "_id");
            if (v == null)
                v = GPLViewModel.getProp(t.parent.id).upi();
            else
                v = v.Value();
        }
        if (this.parent.NAME == "draw2d.shape.node.ConstantValueObj")
           v = this.value;

        //console.log(s.propertyRepresented, v);
        GPLViewModel.setSourceTarget(t.parent.id, s.parent.id, s.propertyRepresented, v);
        ...
        //console.log(t.parent.minRequiredConnection, s.parent.minRequiredConnection);
        //When dragged from Left to Right
        //Source
        window.app.view.updateFigure(t.parent.id, true);
        //Target
        window.app.view.updateFigure(s.parent.id, false);

        //In the situation where we go from left to right and right happens to be
        // Control object, then we re-run updateFigure with sourcePort as false.
        if (t.parent.shapeCat == "Control"){
            window.app.view.updateFigure(t.parent.id, false);
        }

        if (this.parent.NAME == "draw2d.shape.node.ConstantValueObj"){
            //GPLViewModel.getProp(this.parent.id).isConstantEnumAllowed(GPLViewModel.getProp(s.parent.id).isConstantEnumAllowed());
            //GPLViewModel.getProp(this.parent.id).getPointDetails(s.parent.getPort(s.parent.id + "_1").value);
            GPLViewModel.getProp(this.parent.id).constantWorks(s);
        }
        else if (this.parent.NAME == "draw2d.shape.node.ConstantInternalReference"){
            var internalRefId = this.parent.props()["Internal Reference"].Value;
            GPLViewModel.getProp(this.parent.id).constantWorks(s, internalRefId);
        ...
        else
        {
            var self = this;
            if (v > 0){
                $.ajax({ url: '/api/points/' + v, type:'get', async: true, cache:false,
                         success:function(refP) {
                             //console.log(refP);
                             //console.log(v, refP,s.parent.props());
                             GPLViewModel.getProp(s.parent.id).previousVersion(JSON.stringify(s.parent.getPersistentAttributes().props));
                             var d = {point : s.parent.props(), oldPoint:s.parent.props(), refPoint:refP, property:s.propertyRepresented };
                             var dd = {};
                             try{
                                 dd = Config.Update.formatPoint(d);
                             }
                             catch(e){
                                 self.parent.getCanvas().getCommandStack().undo();
                                 iToast.showError("Error", "Cannot resolve a line segment due to value type mismatch.")
                             }
                             GPLViewModel.getProp(s.parent.id).setProps(dd);
                             GPLViewModel.getProp(s.parent.id).updated(true);
