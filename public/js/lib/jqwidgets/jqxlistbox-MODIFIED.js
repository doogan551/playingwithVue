...
(function ($) {
    ...
    $.extend($.jqx._jqxListBox.prototype, {
        ...
        // JDR 3.31.2015 Modify to receive event
        // checks a specific item by its index.
        // checkIndex: function (index, render, raiseEvent) {
        checkIndex: function (index, render, raiseEvent, originalEvent) {
            if (!this.checkboxes) {
                return;
            }
            ...
            if (raiseEvent == undefined || raiseEvent == true) {
                // JDR 3.31.2015 // Modify to send original event
                // this._raiseEvent(3, { label: item.label, value: item.value, checked: true, item: item });
                this._raiseEvent(3, { label: item.label, value: item.value, checked: true, item: item, originalEvent: originalEvent });
            }
        },
        ...
        uncheckIndex: function (index, render, raiseEvent, originalEvent) {
            if (!this.checkboxes) {
                return;
            }
            ...
            if (raiseEvent == undefined || raiseEvent == true) {
                ...
                this._raiseEvent(3, { label: item.label, value: item.value, checked: false, item: item, originalEvent: originalEvent });
            }
        },
        ...
        indeterminateIndex: function (index, render, raiseEvent, originalEvent) {
            if (!this.checkboxes) {
                return;
            }
            ...
            if (raiseEvent == undefined || raiseEvent == true) {
                ...
                this._raiseEvent(3, { checked: null, originalEvent: originalEvent });
            }
        },
        ...
        _handleKeyDown: function (event) {
            ...
            if (key == 32 && this.checkboxes) {
                ...
                if (checkItem != null) {
                    // JDR 3.31.2015 Modified to send event to _updateItemCheck
                    // self._updateItemCheck(checkItem, index);
                    self._updateItemCheck(checkItem, index, event);
        ...
        // JDR 3.31.2015 Modified to receive event argument
        // _updateItemCheck: function (checkItem, index) {
        _updateItemCheck: function (checkItem, index, event) {
            ...
            switch (checkItem.checked) {
                case true:
                    // JDR 3.31.2015 Modified to send event to checkIndex
                    // this.checkIndex(index);
                    this.checkIndex(index, undefined, undefined, event);
                    break;
                case false:
                    // JDR 3.31.2015 Modified to send event to uncheckIndex
                    // this.uncheckIndex(index);
                    this.uncheckIndex(index, undefined, undefined, event);
                    break;
                default:
                    // JDR 3.31.2015 Modified to send event to indeterminateIndex
                    // this.indeterminateIndex(index);
                    this.indeterminateIndex(index, undefined, undefined, event);
                    break;
            }
        },
                        ...
                        if (touch != undefined) {
                            ...
                            if (item != null && !item.isGroup) {
                                ...
                                if (self.checkboxes) {
                                    ...
                                    self._updateItemCheck(item, item.visibleIndex, event);
                                    return;
                                }
                        ...
                        if (event.target.id != ('listBoxContent' + self.element.id) && self.itemswrapper[0] != event.target) {
                            ...
                            if (item != null && !item.isGroup) {
                                var doSelection = function (item, event) {
                                    ...
                                    if (!self.checkboxes) {
                                        //modified to support context menus
                                        //Chris Ellenburg 9/9/2014
                                        //self.selectIndex(item.visibleIndex, false, true, false, 'mouse', event);
                                        self.selectIndex(item.visibleIndex, false, true, true, 'mouse', event);
                                    } else {
