    ...
    $.extend($.jqx._jqxDropDownList.prototype, {
        ...
        render: function () {
            ...
            var comboStructure = $("<div style='background-color: transparent; -webkit-appearance: none; outline: none; width:100%; height: 100%; padding: 0px; margin: 0px; border: 0px; position: relative;'>" +
                ...
                "<div id='dropdownlistContent' unselectable='on' style='outline: none; background-color: transparent; border: none; float: left; position: relative;'/>" +
            ...
            if (this.host.attr('tabindex')) {
                comboStructure.attr('tabindex', this.host.attr('tabindex'));
                this.host.removeAttr('tabindex');
            }
            else {
                comboStructure.attr('tabindex', 0);
            }
            ...
            try {
                ...
                autoBind: this.autoBind, _checkForHiddenParent: false, focusable: this.focusable,
                ...
                this.addHandler(this.listBoxContainer, 'change', function (event) {
                    if (event.args.type != "keyboard") {
                        ...
                        if (!me.isOpened()) {
                            me._raiseEvent('4', { index: me.selectedIndex, type: 'keyboard', item: me.getItem(me.selectedIndex) });
                        }
                    }
                });
               ...
               if (this.focusable && this.isOpened()) {
                ...
                if (this.checkboxes) {
                    ...
                    if (this.width && this.width !== 'auto') {
                        if (notPercentageWidth) {
                            if (!this.rtl) {
                                spanElement.css('max-width', this.host.width() - this.arrowSize - 3);
                            }
                        }
                        ...
                        if (!this.rtl) {
                            if (notPercentageWidth) {
                                spanElement.css('width', this.host.width() - this.arrowSize - 3);
                            }
                        }
                        spanElement.css('text-overflow', 'ellipsis');
                    }
