...
(function ($) {
    ...
    $.extend($.jqx._jqxDataTable.prototype, {
        defineInstance: function () {
            ...
            this.editSettings = { saveOnPageChange: true, saveOnBlur: true, saveOnSelectionChange: true, cancelOnEsc: true, saveOnEnter: true, editSingleCell: false, editOnDoubleClick: true, editOnF2: true };
                ...
                for (var i = 0; i < rows.length; i++) {
                    ...
                    for (var j = 0; j < columns.length; j++) {
                        ...
                        if (i === 0) {
                            ...
                            if (this.columns[j] && this.columns[j].cellsFormat) {
                                var format = this.columns[j].cellsFormat.toLowerCase();
                        ...
                        if (type) {
                            var value = this.getvaluebytype($.trim(cell.text()), dataFields[j]);
            ...
            this.addHandler(this.table, eventname, function (event) {
                ...
                if (key !== undefined) {
                    ...
                    if (rowdata) {
                        ...
                        that._raiseEvent('rowClick', {index: displayIndex, boundIndex: rowIndex, key: key, row: rowdata.row, originalEvent: event });
                        ...
                        if (that._lastSelectedKey == key && (time - that.clickTime.getTime() < timeInterval)) {
                            that._raiseEvent('rowDoubleClick', { index: displayIndex, boundIndex: rowIndex, key: key, row: rowdata.row, originalEvent: event });
                            doubleClick = true;
                            if (event.preventDefault) {
                                event.preventDefault();
                            }
        ...
        setcellvaluebykey: function (key, datafield, value) {
            ...
            var displayIndex = this.getrowdisplayindex(row);
            this._raiseEvent('cellValueChanged', { value: value, oldValue: oldvalue, key: key, boundIndex: index, index: displayIndex, row: this.rowsByKey[key] });
            this._renderrows();
        },
        ...
        getrowdisplayindex: function (row) {
            var index = this.getView().indexOf(row);
            if (index != -1)
                return index;
            ...
            var dataTableRows = this.getView();
            for (var i = 0; i < dataTableRows.length; i++) {
        ...
        endroweditbykey: function (key, cancel) {
            ...
            this.beginupdate();
            var displayIndex = this.getrowdisplayindex(row);
            this._raiseEvent('rowEndEdit', { key: key, index: displayIndex, boundIndex: index, row: this.rowsByKey[key] });
            if (this.editSettings.editSingleCell) {
                var row = this.rowsByKey[key];
                var value = null;
                var displayValue = null;
                if (row) {
                    value = row[this.editColumn.datafield];
                    displayValue = row[this.editColumn.displayfield];
                }

                this._raiseEvent('cellEndEdit', { value: value, displayValue: displayValue, key: key, index: displayIndex, dataField: this.editColumn.datafield, displayField: this.editColumn.displayfield, boundIndex: index, row: this.rowsByKey[key] });
            }
            this.endupdate(false);
