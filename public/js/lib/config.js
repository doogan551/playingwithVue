...
var Config = (function (obj) {
                ...
                if (enumObject) {
                    ...
                    if (val !== undefined) {
                        if (typeof val === 'number') {
                            obj[val] = key;
                        } else if (typeof val.enum === 'number') {
    ...
    obj.Utility = {
        ...
        pointTypes: (function () {
            var types = enumsTemplatesJson.Enums['Point Types'],
                ...
                _getUIEndpoint = function (pointType, id) {
                    ...
                    if (typeof types[pointType] === 'undefined') {
                        throw new Error('Unrecognized Point Type');
                    }
        ...
        setDisplayable: function (point, props, val) {
        ...
        checkValueOptions: function (prop) {
            ...
            prop.eValue = options[key];
        },

        setupPropValueOptions: function (prop, options) {
            prop.isDisplayable = true;
            prop.ValueOptions = options;
        ...
        setChannelOptions: function (prop, min, max) {
            ...
            obj.Utility.setupPropValueOptions(prop, options);
        },
        ...
        checkPointDeviceRMU: function (point) {
            ...
            if ((obj.revEnums['Device Model Types'][point._devModel] === undefined) || (point._devModel === deviceEnums['Central Device'].enum) || (point._devModel === deviceEnums.Unknown.enum)) {
                ...
                switch (point['Point Type'].eValue) {
                    ...
                    case eTyp.Accumulator.enum:
                        switch (point._devModel) {
                            case deviceEnums['MicroScan 5 UNV'].enum:
                            case deviceEnums['SCADA Vio'].enum:
                            case deviceEnums['SCADA IO'].enum:
                            case deviceEnums['MicroScan 4 UNV'].enum:
                            case deviceEnums['MicroScan 4 Digital'].enum:
                                break;
                    ...
                    case eTyp['Binary Input'].enum:
                        switch (point._devModel) {
                            case deviceEnums['MicroScan 5 xTalk'].enum:
                            case deviceEnums['MicroScan 4 xTalk'].enum:
                                return eRel;
        ...
        getRmuValueOptions: function (devModel) {
            ...
            switch (devModel) {
                ...
                case deviceEnums['MicroScan 4 UNV'].enum:
                    return {
                        'MS3 RT': rmuEnums['MS3 RT'].enum,
                        'MS 3 EEPROM': rmuEnums['MS 3 EEPROM'].enum,
                        'MS 3 Flash': rmuEnums['MS 3 Flash'].enum,
                        'MS 4 VAV': rmuEnums['MS 4 VAV'].enum,
                        'BACnet': rmuEnums.BACnet.enum,
                        'Programmable Modbus': rmuEnums['Programmable Modbus'].enum,
                        'N2 Device': rmuEnums['N2 Device'].enum,
                        'Liebert': rmuEnums.Liebert.enum,
                        'Sierra Steam Meter': rmuEnums['Sierra Steam Meter'].enum,
                        'Siemens Power Meter': rmuEnums['Siemens Power Meter'].enum,
                        'Ingersol Rand Intellysis': rmuEnums['Ingersol Rand Intellysis'].enum,
                        'PowerLogic 3000 Meter': rmuEnums['PowerLogic 3000 Meter'].enum,
                        'Generic Modbus': rmuEnums['Generic Modbus'].enum,
                        'PowerTraks 9000': rmuEnums['PowerTraks 9000'].enum
                    };

                case deviceEnums['MicroScan Interface Device'].enum:
                    return {
                        'MS3 RT': rmuEnums['MS3 RT'].enum,
                        'MS 3 EEPROM': rmuEnums['MS 3 EEPROM'].enum,
                        'MS 3 Flash': rmuEnums['MS 3 Flash'].enum
                ...
                case deviceEnums['Field Interface Device'].enum:
                    return {
                        'IFC Remote Unit': rmuEnums['IFC Remote Unit'].enum
                    };

                case deviceEnums['Staefa Interface Device'].enum:
                    return {
                        'Smart II Remote Unit': rmuEnums['Smart II Remote Unit'].enum
                    };

                case deviceEnums['N2 Interface Device'].enum:
                    return {
                        'N2 Device': rmuEnums['N2 Device'].enum
                    };

                case deviceEnums['Sierra Steam Meter Device'].enum:
                    return {
                        'Sierra Steam Meter': rmuEnums['Sierra Steam Meter'].enum
                    };

                case deviceEnums['Armstrong SteamEye Device'].enum:
                    return {
                        'ASE Remote Unit': rmuEnums['ASE Remote Unit'].enum
                    };

                case deviceEnums['Siemens Power Meter Device'].enum:
                    return {
                        'Siemens Power Meter': rmuEnums['Siemens Power Meter'].enum
                    };

                case deviceEnums['Ingersol Rand Intellysis Device'].enum:
                    return {
                        'Ingersol Rand Intellysis': rmuEnums['Ingersol Rand Intellysis'].enum
                    };

                case deviceEnums['BACnet Interface Device'].enum:
                    return {
                        'BACnet': rmuEnums.BACnet.enum
                    };
    ...
    obj.Update = {
        ...
        formatPoint: function (data) {
            // display dyn, displayable anim, displayable button, displayable trend, slide display, point reg
            ...
            // If no errors encountered so far
            if (data.ok === true) {
                if (valueType === enumsTemplatesJson.Enums['Value Types'].UniquePID.enum) {
            ...
            // If the property validation succeeded
            if (data.ok === true) {
    ...
    obj.EditValidation = {
        ...
        'Control Band Value': function (data) {
            var val = data.propertyObject.Value;
        ...
        'Downlink Network': function (data) {
            var point = data.point,
                type = point['Point Type'].Value; // Property value
        ...
        'Model Type': function (data) {
            ...
            } else if (point['Point Type'].Value === 'Device') {
    ...
    obj.EditChanges = {
        ...
        updateFanStrategy: function (point) {
            ...
            if (obj.Utility.getPropertyObject('Fan Control Point', point).Value !== 0) {
                ...
                } else if ((fanOffSetPt > 0) && (fanOffSetPt < point['Occupied Min Cool CFM'].Value)) {
                    strategy = 'Deadband';
                }
        ...
        'Broadcast Enable': function (data) {
            ...
            if (prop) {
                prop.isDisplayable = isDisplayable;
            }
            return point;
        },
        ...
        'Calculation Type': function (data) {
            ...
            if ((point['Point Type'].Value === 'Analog Selector') || (point['Point Type'].Value === 'Binary Selector')) {
                ...
                setDisplayable(point, ['Rise Time', 'Fall Time'], true);
                setDisplayable(point, ['Proportional Band', 'Reset Gain'], false);
            } else if (val === 'PI') {
                setValueOptions(point['Input Range'], {
                    '1': 0,
                    '10': 1,
                ...
                setDisplayable(point, ['Proportional Band', 'Reset Gain'], true);
                setDisplayable(point, ['Rise Time', 'Fall Time'], false);
            } else {
                point['Proportional Band'].isDisplayable = true;
                setDisplayable(point, ['Rise Time', 'Fall Time', 'Input Range', 'Reset Gain'], false);
            }
            return point;
        },
        ...
        'Enable Warning Alarms': function (data) {
            ...
            if (point['Enable Warning Alarms'].Value === true) {
                setDisplayable(point, ['High Warning Limit', 'Low Warning Limit'], true);
                ...
                setDisplayable(point, ['High Warning Limit', 'Low Warning Limit', 'Warning Adjust Band'], false);
            }
            return point;
        },
        ...
        applyComparatorInputTypeN: function (data) {
            ...
            } else if (obj.Utility.getPropertyObject('Input Point 2', point).PointInst === 0) {
        ...
        'Trend Samples': function (data) {
            var point = data.point,
                ...
                len = props.length,
                displayable = (point[data.property].Value !== 0) ? true : false,
                prop,
                i;
            ...
            for (i = 0; i < len; i++) {
                prop = props[i];
                if (point.hasOwnProperty(prop)) {
                    point[prop].isDisplayable = displayable;
                }
            }
            return point;
        },
        ...
        applyDeviceEthernetProtocol: function (data) {
            ...
            if ((point['Ethernet Protocol'].Value === 'IP') && (point['Ethernet Protocol'].isDisplayable === true)) {
                ...
                if (obj.Utility.checkMicroScan5Device(point)) {
                    setDisplayable(point, ['Ethernet Address', 'Ethernet IP Port', 'Ethernet Gateway', 'Ethernet Subnet', 'Ethernet Network'], true);
                } else {
                    setDisplayable(point, ['Ethernet Address', 'Ethernet IP Port'], ro);
                    setDisplayable(point, ['Ethernet Gateway', 'Ethernet Subnet'], false);
                }
            } else {
                setDisplayable(point, ['Ethernet Address', 'Ethernet IP Port', 'Ethernet Gateway', 'Ethernet Subnet', 'Ethernet Network'], false);
            }
            return point;
        },
        ...
        applyDeviceDownlinkProtocol: function (data) {
            var point = data.point,
                displayable = ((point['Downlink Protocol'].Value === 'IP') && (point['Downlink Protocol'].isDisplayable === true)) ? true : false;

            point['Downlink IP Port'].isReadOnly = (point['Downlink Network'].Value !== 0) ? true : false;
            obj.Utility.setDisplayable(point, ['Downlink Broadcast Delay', 'Downlink IP Port', 'Downlink Network'], displayable);
            return point;
        },
        ...
        applyDeviceDevModel: function (data) {
            ...
            if (obj.Utility.checkMicroScan5Device(point)) {
                ...
                setDisplayable(point, ['Ethernet Protocol', 'Port 1 Protocol', 'Port 2 Protocol'], true);
                setDisplayable(point, ['Port 3 Protocol', 'Port 4 Protocol'], false);
            } else if (point['Model Type'].Value === 'Central Device') {
                setValueOptions(upPort, {
                    'Ethernet': enums['Device Ports'].Ethernet.enum
                });
                setDisplayable(point, ['Port 1 Protocol', 'Port 2 Protocol', 'Port 3 Protocol', 'Port 4 Protocol'], false);
                ...
                setDisplayable(point, ['Ethernet Protocol', 'Port 2 Protocol', 'Port 3 Protocol', 'Port 4 Protocol'], false);
            }
        ...
        applyRemoteUnitNetworkType: function (point) {
            ...
            if (point.Instance.isDisplayable) {
                ...
                if (nt === 'Unknown') {
                    setDisplayable(point, ['Device Address', 'Network Segment', 'Ethernet IP Port', 'Router Address'], false);
                } else {
                    setDisplayable(point, ['Device Address', 'Network Segment'], true);
                ...
                setDisplayable(point, ['Network Segment', 'Router Address'], false);
                if (point['Device Port'].Value === 'Ethernet') {
                    setDisplayable(point, ['Modbus Unit Id', 'Ethernet IP Port'], true);
                } else {
                    setDisplayable(point, ['Modbus Unit Id', 'Ethernet IP Port'], false);
                }
            }
            return point;
        },
        ...
        applyRemoteUnitDevModel: function (data) {
            ...
            if ((point._devModel === enums['Device Model Types']['Central Device'].enum) || (point._devModel === enums['Device Model Types'].Unknown.enum)) {
                ...
                if (obj.Utility.checkMicroScanRMU(point)) {
                    setDisplayable(point, ['Configure Device', 'Firmware Version'], true);
                    ...
                    if (obj.Utility.checkModbusRMU(point)) {
                        ...
                        if (modelType === 'Programmable Modbus') {
                            setDisplayable(point, ['Poll Function', 'Poll Register'], true);
                        }
                    } else if (modelType === 'BACnet') {
                        setDisplayable(point, ['Instance', 'Network Type'], true);
                        if (ms5Dev) {
        ...
        applyConversionTypeProperty: function (data) {
            ...
            displayable = propertyObject.isDisplayable;
            ro = propertyObject.isReadOnly;
            for (var i = 1; i < 5; i++) {
                point[prop + i].isDisplayable = displayable;
        ...
        applyAnalogInputDevModel: function (data) {
            ...
            if (point._relPoint === enumsTemplatesJson.Enums.Reliabilities['No Fault'].enum) {
                switch (point._rmuModel) {
                    case rmuEnums['Programmable Modbus'].enum:
                    case rmuEnums.Liebert.enum:
                    case rmuEnums['Sierra Steam Meter'].enum:
                    case rmuEnums['Siemens Power Meter'].enum:
                    case rmuEnums['Ingersol Rand Intellysis'].enum:
                    case rmuEnums['PowerLogic 3000 Meter'].enum:
                    case rmuEnums['Generic Modbus'].enum:
                    case rmuEnums['PowerTraks 9000'].enum:
                        obj.Utility.setModbusPropsDisp(point);
                        break;

                    case rmuEnums.BACnet.enum:
                    ...
                    case rmuEnums['MS 4 VAV'].enum:
                        setValueOptions(inType, {
                            'Normal': 0,
                            'Inverted': 2,
                            'Velocity': 3
                        });
                        setValueOptions(channel, {
                    ...
                    case rmuEnums['MS 3 Flash'].enum:
                        ...
                        setChannelOptions(channel, 0, 7);
                        break;

                    case rmuEnums['N2 Device'].enum:
                        point.Instance.isDisplayable = true;
                        break;

                    case rmuEnums['IFC Remote Unit'].enum:
                        setValueOptions(inType, {
                            'High Resistance': 0,
                            'High Voltage': 1,
                            'Low Resistance': 2,
                        ...
                        setChannelOptions(channel, 0, 15);
                        break;

                    case rmuEnums['Smart II Remote Unit'].enum:
                        setChannelOptions(channel, 0, 7);
                        break;
                    ...
                    default: // Unknown, no RMU
                        switch (point._devModel) {
                            case deviceEnums['MicroScan 5 UNV'].enum:
                                ...
                                setChannelOptions(channel, 1, 16);
                                break;
                            case deviceEnums['SCADA Vio'].enum:
                                setValueOptions(inType, {
                                    '20 mA SP': 3,
                            ...
                            default: // MicroScan 4 Digital
                                ...
                                setChannelOptions(channel, 1, 32);
                                break;
                        }
                        break;
                }
                    ...
                    default: // Unknown, no RMU
                        ...
                        switch (point._devModel) {
                            ...
                            default: // MicroSPC Device
                                setChannelOptions(channel, 0, 0);
                                channel.isDisplayable = false;
        ...
        applyAnalogOutputOutputType: function (data) {
            ...
            obj.Utility.setDisplayable(point, ['Channel', 'Open Channel', 'Close Channel', 'Polarity'], false);

            if (point['Output Type'].isDisplayable) {
                ...
                if (max >= 0) {
                    if (val === 'Pulsed') {
                        setChannelOptions(openChannel, min, max);
                        setChannelOptions(closeChannel, min, max);
                    } else {
                        setChannelOptions(channel, min, max);
                    }
                }
            }
            return point;
        ...
        applyAnalogOutputDevModel: function (data) {
            ...
            if (point._relPoint === enumsTemplatesJson.Enums.Reliabilities['No Fault'].enum) {
                switch (point._rmuModel) {
                    ...
                    case rmuEnums['MS 3 Flash'].enum:
                    case rmuEnums['IFC Remote Unit'].enum:
                        setValueOptions(outputType, options);
                        break;
                ...
                setDisplayable(point, ['Conversion Coefficient 1', 'Conversion Coefficient 2'], sensorPoint.isDisplayable);
            }
            return point;
        },
        ...
        applyBinaryOutputTypeFeedbackType: function (data) {
            var point = data.point,
                setDisplayable = obj.Utility.setDisplayable,
                feedbackPoint = obj.Utility.getPropertyObject('Feedback Point', point);

            feedbackPoint.isDisplayable = false;
            setDisplayable(point, ['Feedback Channel', 'Feedback Instance', 'Feedback Polarity', 'Open Channel', 'Open Polarity', 'Close Channel', 'Close Polarity', 'Supervised Input'], false);

            switch (point['Feedback Type'].Value) {
                case 'Single':
                    if (point.Instance.isDisplayable) {
                        setDisplayable(point, ['Feedback Instance', 'Feedback Polarity'], true);
                    } else {
                        setDisplayable(point, ['Feedback Channel', 'Feedback Polarity'], true);
                ...
                case 'Dual':
                    setDisplayable(point, ['Open Channel', 'Open Polarity', 'Close Channel', 'Close Polarity'], true);
                    break;

                case 'Point':
                    point['Feedback Polarity'].isDisplayable = true;
                    feedbackPoint.isDisplayable = true;
                    break;
        ...
        applyBinaryOutputOutputType: function (data) {
            ...
            if (point._rmuModel === enumsTemplatesJson.Enums['Remote Unit Model Types']['MS 4 VAV'].enum) {
                ...
                obj.Utility.setDisplayable(point, ['On Channel', 'Off Channel', 'Momentary Delay'], displayable);
            }
            return point;
        },
        ...
        applyBinaryOutputDevModel: function (data) {
            ...
            if (point._relPoint === enumsTemplatesJson.Enums.Reliabilities['No Fault'].enum) {
                switch (point._rmuModel) {
                    ...
                    case rmuEnums['N2 Device'].enum:
                        point.Instance.isDisplayable = true;
                        setValueOptions(feedbackType, feedbackOptions);
                        break;
                    ...
                    case rmuEnums['MS 4 VAV'].enum:
                        ...
                        feedbackType.isDisplayable = false;
                        setValueOptions(point.Channel, {
                            '3 - Lights': 3,
                            '5 - Fan': 5,
                            '6 - Digital Heat 1': 6,
                            '7 - Digital Heat 2': 7,
                    ...
                    case rmuEnums['Smart II Remote Unit'].enum:
                        ...
                        outputType.isDisplayable = false;
                        point.Polarity.isDisplayable = true;
                        setValueOptions(feedbackType, feedbackOptions);
                        setChannelOptions(point.Channel, 0, 7);
                        feedbackMin = 0;
                        feedbackMax = 7;
                        break;
                    ...
                    default: // Unknown, no RMU
                        ...
                        switch (point._devModel) {
                            ...
                            case deviceEnums['SCADA IO'].enum:
                                setValueOptions(point.Channel, ioOptions);
                                setValueOptions(point['On Channel'], ioOptions);
                                setValueOptions(point['Off Channel'], ioOptions);
                                setValueOptions(point['Feedback Channel'], ioOptions);
                                setValueOptions(point['Open Channel'], ioOptions);
                                setValueOptions(point['Close Channel'], ioOptions);
                                break;
                ...
                if (outputType.isDisplayable) {
                    point.Polarity.isDisplayable = true;
                    if (channelMax >= 0) {
                        setChannelOptions(point.Channel, channelMin, channelMax);
                        setChannelOptions(point['On Channel'], channelMin, channelMax);
                        setChannelOptions(point['Off Channel'], channelMin, channelMax);
                    }
                    obj.EditChanges.applyBinaryOutputOutputType(data);
                }
                if (feedbackMax >= 0) {
                    setChannelOptions(point['Feedback Channel'], feedbackMin, feedbackMax);
                    setChannelOptions(point['Open Channel'], feedbackMin, feedbackMax);
                    setChannelOptions(point['Close Channel'], feedbackMin, feedbackMax);
                }
        ...
        applyAnalogValueMonitorPoint: function (data) {
            var point = data.point,
                monitorPoint = obj.Utility.getPropertyObject('Monitor Point', point),
                setDisplayable = obj.Utility.setDisplayable;

            if (monitorPoint.isDisplayable && (monitorPoint.PointInst !== 0)) {
                setDisplayable(point, ['Fail Action', 'Demand Enable'], true);
                point['Demand Interval'].isDisplayable = point['Demand Enable'].Value;
            } else {
                setDisplayable(point, ['Fail Action', 'Demand Enable', 'Demand Interval'], false);
            }
            return point;
        },
        ...
        applyAccumulatorDevModel: function (data) {
            ...
            setDisplayable(point, ['Channel', 'Instance', 'Read Only', 'Modbus Order', 'Poll Register', 'Poll Data Type', 'Poll Function', 'Fast Pulse', 'Rate Period'], false);
            point._relPoint = obj.Utility.checkPointDeviceRMU(point);
            if (point._relPoint === enumsTemplatesJson.Enums.Reliabilities['No Fault'].enum) {
                switch (point._rmuModel) {
                    case rmuEnums['Programmable Modbus'].enum:
                    ...
                    default: // Unknown, no RMU
                        switch (point._devModel) {
                            case deviceEnums['MicroScan 5 UNV'].enum:
                                setDisplayable(point, ['Fast Pulse', 'Rate Period'], true);
                                setChannelOptions(channel, 1, 16);
                                break;
                            case deviceEnums['SCADA Vio'].enum:
                                setDisplayable(point, ['Fast Pulse', 'Rate Period'], true);
                                setChannelOptions(channel, 6, 9);
                                break;
                            case deviceEnums['SCADA IO'].enum:
                                setDisplayable(point, ['Fast Pulse', 'Rate Period'], true);
                                setValueOptions(channel, {
        ...
        applyCpcPointDevModel: function (point) {
            ...
            point['Maximum Change'].isDisplayable = displayable;
        },
        ...
        applyProportionalDevModel: function (data) {
            ...
            obj.Utility.setupPropValueOptions(point['Calculation Type'], options);
            point['Maximum Change'].isDisplayable = displayable;
        ...
        applyMathDevModel: function (data) {
            ...
            point.Rate.isDisplayable = displayable;
            return obj.EditChanges.applyEnthalpyDevModel(data);
        },
        ...
        applyLogicDevModel: function (data) {
            ...
            for (i = 1; i <= 5; i++) {
                obj.Utility.setupPropValueOptions(point['If Compare ' + i], options);
            }
            return point;
        }
    };
